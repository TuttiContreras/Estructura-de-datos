...
public:
    ...
    ArbolBinario<T>* getReflexion();
    void getReflexion(NodoB<T>* padre, NodoB<T>* father);
};
...
template <class T>
ArbolBinario<T>* ArbolBinario<T>::getReflexion(){
    NodoB<T>* s= new NodoB<T>(raiz->getInfo());
    ArbolBinario<T>* reflexion = new ArbolBinario(s);
    getReflexion(s, raiz);
    return reflexion;
}

template <class T>
void ArbolBinario<T>::getReflexion(NodoB<T>* padre, NodoB<T>* father){
    NodoB<T>* temp;
    if(father->getDerecho()){
        temp = new NodoB<T>(father->getDerecho()->getInfo());
        padre->setIzquierdo(temp);
        getReflexion(padre->getIzquierdo(), father->getDerecho());
    }
    if(father->getIzquierdo()){
        temp = new NodoB<T>(father->getIzquierdo()->getInfo());
        padre->setDerecho(temp);
        getReflexion(padre->getDerecho(), father->getIzquierdo());
    }
}

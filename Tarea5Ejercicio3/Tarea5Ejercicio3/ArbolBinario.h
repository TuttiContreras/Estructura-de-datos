...
public:
    ...
    bool simetrico();
    bool simetrico(NodoB<T>* i, NodoB<T>* d);
};
...
template <class T>
bool ArbolBinario<T>::simetrico(){
    
    return simetrico(raiz->getIzquierdo(), raiz->getDerecho());
}

template <class T>
bool ArbolBinario<T>::simetrico(NodoB<T>* i, NodoB<T>* d){
    if(i || d){
        if(!i || !d){
            return false;
        }
        else if(i->getInfo() != d->getInfo()){
            return false;
        }
        else if(simetrico(i->getIzquierdo(), d->getDerecho()) && (simetrico(i->getDerecho(),d->getIzquierdo()))){
            return true;
        }
    }
    return true;
